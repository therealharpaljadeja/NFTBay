...
export const fetchItemsCreated = async (wallet) => {
	...
	for (let i = 0; i < result.length; i++) {
		if (result[i].sold != true) {
			...
			// let isApprovedByOwner = await nftContract.isApprovedToMarketplace(process.env.REACT_APP_MARKETPLACE_ADDRESS, result[i].tokenId.toString());
			const creatorsContract = new ethers.Contract(
				process.env.REACT_APP_CREATORS_CONTRACT_REEF,
				Creators.abi,
				wallet
			);
			let creatorAddress =
...
export const createSale = async (wallet, collectionAddress, tokenId, price) => {
	...
	let result = await nftMarketContract.createMarketSale(
		collectionAddress,
		tokenId,
		{ value: price }
	);
...
export const createMarketItem = async (
	...
	let result = await nftMarketContract.createMarketItem(
		collectionAddress,
		tokenId,
		price,
...
export const getMarketItemByItemId = async (wallet, itemId) => {
	...
	// let isApprovedByOwner = await nftContract.isApprovedToMarketplace(process.env.REACT_APP_MARKETPLACE_ADDRESS, result.tokenId.toString());
	const creatorsContract = new ethers.Contract(
		process.env.REACT_APP_CREATORS_CONTRACT_REEF,
		Creators.abi,
		wallet
